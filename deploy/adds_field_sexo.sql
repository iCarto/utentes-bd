-- Deploy utentes:adds_field_sexo to pg

BEGIN;

CREATE TABLE domains.sexo (
    category text DEFAULT 'sexo'
...
ALTER TABLE domains.sexo OWNER TO :owner;
REVOKE ALL ON TABLE domains.sexo FROM PUBLIC;
GRANT SELECT ON TABLE domains.sexo TO :base_user;


INSERT INTO domains.sexo (key, ordering, tooltip) VALUES
       ('Feminino', 0, 'F'), ('Masculino', 1, 'M'), ('Outros', 2, 'Quando o sexo da pessoa não é conhecido ou não se enquadra');


ALTER TABLE utentes.exploracaos ADD COLUMN sexo_gerente
      text not null default 'Outros'
      REFERENCES domains.sexo(key) ON UPDATE CASCADE ON DELETE NO ACTION
;
COMMENT ON COLUMN utentes.exploracaos.sexo_gerente IS 'Sexo do Gerente/Presidente';


ALTER TABLE utentes.utentes ADD COLUMN sexo_gerente
      text not null default 'Outros'
      REFERENCES domains.sexo(key) ON UPDATE CASCADE ON DELETE NO ACTION
;
COMMENT ON COLUMN utentes.utentes.sexo_gerente IS 'Sexo do Gerente/Presidente';
