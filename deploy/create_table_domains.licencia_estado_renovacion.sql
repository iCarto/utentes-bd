-- Deploy utentes:create_table_domains.licencia_estado_renovacion to pg

BEGIN;

CREATE TABLE domains.licencia_estado_renovacao (
    category text not null default 'licencia_estado_renovacao',
    key text UNIQUE,
    value text,
    ordering integer,
    parent text,
...
INSERT INTO domains.licencia_estado_renovacao (key, ordering, tooltip, app) VALUES
    ('Não aprovada', 1, 'A licença foi negada por não cumprir os requisitos', NULL),
    ('Documentação incompleta (Pendente utente - Direcção)', 2, 'O utente tem pendente de entrega alguma documentação solicitada pela Direcção', NULL),
    ('Documentação incompleta (Pendente utente - DA)', 2, 'O utente tem pendente de entrega alguma documentação solicitada pelo D. Administrativo', NULL),
    ('Documentação incompleta (Pendente utente - DJ)', 2, 'O utente tem pendente de entrega alguma documentação solicitada pelo D. Jurídico', NULL),
    ('Documentação incompleta (Pendente utente - DT)', 2, 'O utente tem pendente de entrega alguma documentação solicitada pelo R. Cadastro DT', NULL),
    ('Documentação incompleta (Pendente utente - DF)', 2, 'O utente tem pendente de entrega alguma documentação solicitada pelo D. Finaciero', NULL),
    ('Pendente Renovação da licença (DA)', 3, 'O utente tem entregado o pedido de renovação de licença. Pendente de revisão pelo departamento administrativo', NULL),
    ('Pendente Revisão Renovação (Direcção)', 4, 'O utente tem entregado o pedido de renovação de licença. Pendente de revisão pela direcção', NULL),
    ('Pendente Análise Renovação Licença (DJ)', 5, 'O utente tem entregado o pedido renovação de licença. Pendente de revisão legal pelo departamento jurídico', NULL),
    ('Pendente Parecer Técnico Renovação (DT)', 6, 'Pendente que o departamento técnico emita o parecer da unidade e o parecer técnico.', NULL),
    ('Pendente Emisão Renovação Licença (DJ)', 7, 'Pendente de revisão dos pareceres técnicos e emissão da licença pelo departamento jurídico', NULL),
    ('Pendente Dados Renovação Licença (DJ)', 8, 'Pendente de introcução dos dados da nova licença pelo departamento jurídico', NULL),
    ('Pendente Firma Renovação Licença (Direcção)', 9, 'Licença emitida. Pendente da revisão e assinatura de direcção para sua aprovação definitiva e do pagamento da factura de emissão da licença.', NULL),
    ('Licenciada', 10, 'Licença concedida, em activo e com funcionamento regular', NULL),
    ('Utente de facto', 11, 'Utente que usa e aprovietam a água sem licença ou título que o legitime. Fica sujeito ao pagamento em dobro da taxa.', NULL);
...
WITH tmp_query AS (
       ...
       UNION
       SELECT category, key, value, ordering, parent, tooltip, NULL::text[] FROM domains.licencia_estado_renovacao
        )
