...
('licencia_estado', 'Desconhecido', NULL, 1, NULL, NULL, ARRAY['Cetmar']);
...
UPDATE domains.provincia SET app = array_append(app, 'Cetmar') where key = 'Cabo Delgado';
...
update domains.distrito set app = array_append(app, 'Cetmar') where parent = 'Cabo Delgado';
...
update domains.posto set app = array_append(app, 'Cetmar') where parent IN ( select key from domains.distrito where parent = 'Cabo Delgado' );


ALTER TABLE domains.bacia ADD COLUMN app TEXT[];
update domains.bacia set app = string_to_array(ara, ';');
UPDATE domains.bacia set app = ARRAY['Norte', 'Cetmar'] WHERE ara = 'Norte' AND key != 'Rovuma';
...
UPDATE domains.subacia set app = ARRAY['Norte', 'Cetmar'] WHERE ara = 'Norte' AND key != 'Rovuma';
