-- Deploy utentes:alter_table_utentes_fontes_with_inventario_columns to pg

BEGIN;

UPDATE inventario.fontes SET tip_fonte = NULL WHERE tip_fonte = 'Outros';
UPDATE utentes.fontes SET tipo_fonte = NULL WHERE tipo_fonte = 'Outros';

ALTER TABLE inventario.fontes DROP CONSTRAINT fontes_tip_fonte_fkey;
DROP TABLE inventario_dominios.tip_fonte;

ALTER TABLE inventario.fontes
    ADD FOREIGN KEY (tipo_agua, tip_fonte)
    REFERENCES domains.fonte_tipo(parent, key) ON UPDATE CASCADE
;

DELETE FROM domains.fonte_tipo WHERE key = 'Outros' OR key IS NULL;

ALTER TABLE inventario_dominios.red_monit SET SCHEMA domains;
ALTER TABLE domains.red_monit ALTER COLUMN category SET DEFAULT 'red_monit';
UPDATE domains.red_monit SET category = 'red_monit';

UPDATE inventario.fontes SET red_monit = 'NO' WHERE red_monit IS NULL;
ALTER TABLE inventario.fontes ALTER COLUMN red_monit SET NOT NULL;

ALTER TABLE utentes.fontes
    ADD COLUMN red_monit text REFERENCES domains.red_monit(key) ON UPDATE CASCADE
   , ADD COLUMN bombeo boolean
   , ADD COLUMN prof_pozo numeric(10, 2)
   , ADD COLUMN diametro numeric(10, 2)
;

UPDATE utentes.fontes SET red_monit = 'NO' WHERE red_monit IS NULL;
ALTER TABLE utentes.fontes ALTER COLUMN red_monit SET NOT NULL;


COMMENT ON COLUMN utentes.fontes.red_monit IS 'Red de monitoreo';
COMMENT ON COLUMN utentes.fontes.bombeo IS 'Bombagem';
COMMENT ON COLUMN utentes.fontes.prof_pozo IS 'Profundidade (m)';
COMMENT ON COLUMN utentes.fontes.diametro IS 'Diâmetro interior (m)';
...
WITH tmp_query AS (
       ...
       UNION
         SELECT category, key, value, ordering, parent, tooltip, NULL::text[] AS text
